...
def dkperp_dbll(redshift, cosmo=cosmo100):

    """
    ------------------------------------------------------------------------
    Compute jacobian to transform baseline lengths |u| (in number of 
    wavelengths) to transverse comoving wavenumbers (h/Mpc) corresponding to 
    specified redshift(s)
    ...
    Jacobian to convert baseline lengths (|u|) to k_perp. Same size as redshift
    ------------------------------------------------------------------------
    """
    ...
    jacobian = 2 * NP.pi / cosmo.comoving_transverse_distance(redshift).value
    return jacobian
...
def reflection_attenuation(dps, modelPSinfo, redshifts, kprll_cut, bll,
    ...
    if fgmdl_power_shape[1] > 1:
        warnings.warn('Multiple spectral windows found. Onlt the first will be used as reference for foreground power spectra')
    zind_FG = 0
    ...
    conversion_factor_kperp_from_bll = dkperp_dbll(redshifts, cosmo=cosmo)
    conversion_factor_kprll_from_delay = dkprll_deta(redshifts, cosmo=cosmo)
