...
rootdir = '/data3/t_nithyanandan/project_SKA/'
beams_dir = 'power_patterns/Eloy/lfaa_256_af_fits/'
prefix = 'lfaa_256_af_'
suffix = '.fits'
pols = ['X']
...
for pol in pols:
    ...
    freqs_str = [fname.split('_')[3] for fname in fnames]
...
outfile = rootdir + beams_dir + '{0[0]}_{0[1]}_{0[2]}'.format(fnames[0].split('_')[:3])+suffix
hdulist = []
hdulist += [fits.PrimaryHDU()]
hdulist[0].header['EXTNAME'] = 'PRIMARY'
hdulist[0].header['NPOL'] = (len(pols), 'Number of polarizations')
hdulist[0].header['SOURCE'] = ('SKA-HARP', 'Source of data')
