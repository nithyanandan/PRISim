...
for i in range(n_bins_baseline_orientation):
    ax = fig.add_subplot(2,3,blo_ax_mapping[i])
...
def update(i, pointing_radec, lst, obsmode, telescope, backdrop_coords, bll, blori, lags, vis_lag, delaymatrix, overlays, xv, yv, xv_uniq, yv_uniq, axs, tpc):
    ...
    for j in range(len(axs)-1):
        ...
        axs[j].lines[0].set_ydata(delay_ranges[i,blind[sortind],0]*1e6)
        ...
        axs[j].lines[2].set_ydata(delay_horizon[i,blind[sortind],0]*1e6)
...
PLT.show()
animation_file = '/data3/t_nithyanandan/project_MWA/multi_baseline_noiseless_visibilities_'+obs_mode+'_'+'{0:0d}'.format(20*baseline_chunk_size)+'_baselines_{0:0d}_orientations_'.format(n_bins_baseline_orientation)+'gaussian_FG_model_'+fg_str+'_{0:0d}_'.format(nside)+bpass_shape+'{0:.1f}'.format(oversampling_factor)
